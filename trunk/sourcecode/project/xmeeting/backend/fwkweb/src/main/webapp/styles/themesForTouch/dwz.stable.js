...
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			var customHeight = $(this).attr("customHeight");
			var customHeightStr = customHeight ? " customHeight='" + customHeight + "'" : "";
			var heightStr = customHeight ? " height:" + customHeight + "px;" : "";
			
			tbody.wrap("<div class='gridScroller2'" + customHeightStr+" "+layoutStr + " style='width:" + $tc.width() + "px; "+heightStr+"'><div class='gridTbody'><table style='width:" + (tlength - 20) + "px;'></table></div></div>");
						...
						if($th.outerWidth() - offset < 5) {
							$th.css("cursor", "col-resize").mousedown(function(event){
								...
								$(".resizeProxy", $grid).jDrag($.extend(options, {scop:true, cellMinW:20, relObj:$(".resizeMarker", $grid)[0],
										...
										//	$th.width(newW+"px");
												$("tr", thead).each(function (){//多行thead
											    
											     $("th", this).eq(cellNum - 1).width(newW+"px");
											    
											}); 
											
									
											$(">tr", $(tbody)).each(function(){
												var $dcell=$(">td", $(this)).eq(cellNum - 1);
												$dcell.width(newW+"px");
											});
