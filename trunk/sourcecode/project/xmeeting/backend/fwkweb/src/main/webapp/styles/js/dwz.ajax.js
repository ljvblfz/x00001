 ...
 */
 ...
 /**
 *对string类型form action属性
 *作为js语句执行，获取重新设定action属性
 */
 function evaluteFormAction($form) {
 	if($form.attr('action').indexOf("javascript:") != -1){
		var fun = new Function('return ' + $form.attr('action').replaceAll("javascript:",""));
		var ac = fun();
		$form.attr('action',ac);
	}
	return $form;
...
function validateCallback(form, callback) {
	var $form = getJqueryForm(form);
	if (!$form.valid()) {
		return false;
	}
	$form = evaluteFormAction($form);
	$.ajax({
 ...
 * 普通ajax表单提交
 * @param {Object} form
 * @param {Object} beforeValidMethod--其他的验证方法
 * @param {Object} callback
 */
function validateCallbackWithBefore(form,beforeValidMethod, callback) {
	var $form = getJqueryForm(form);
	
	if(!beforeValidMethod()){
		return false;
	}
	if (!$form.valid()) {
		return false;
...
function navTabAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		if(json.callBackMethod){//执行成功后调用页面返回的JS方法
			window[json.callBackMethod]();   
		}else if(json.navTabId){ //把指定navTab页面标记为需要“重新载入”。注意navTabId不能是当前navTab页面的
...
function dialogAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		if(json.callBackMethod && json.callBackMethod!=""){
			window[json.callBackMethod]();   
		}
	}
}
...
function doGetAjax(url,opts, callback){
	var $callback = callback || navTabAjaxDone;
	...
	$.ajax({
		type:"GET",
		url:url,
		dataType:opts?(opts.returnType||"json"):"json",
		cache: false,
		data:opts,
		success: $callback,
		error: DWZ.ajaxError
...
function doPostAjax(url,opts, callback){
	showLoading=true;
	var $callback = callback || navTabAjaxDone;
	if (! $.isFunction($callback)) $callback = eval('(' + callback + ')');
	$.ajax({
...
function doDeleteAjax(url,opts, callback){
	showLoading=true;
	var $callback = callback || navTabAjaxDone;
	if (! $.isFunction($callback)) $callback = eval('(' + callback + ')');
	$.ajax({
...
function doPutAjax(url,opts, callback){
	showLoading=true;
	var $callback = callback || navTabAjaxDone;
	if (! $.isFunction($callback)) $callback = eval('(' + callback + ')');
	$.ajax({
