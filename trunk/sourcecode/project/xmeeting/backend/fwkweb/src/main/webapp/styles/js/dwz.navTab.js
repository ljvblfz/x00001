...
var navTab = {
	...
	_contextmenu:function($obj){ // navTab右键菜单
		...
		$obj.contextMenu('navTabCM', {
			...
			ctrSub:function(t,m){
				...
				if(t.attr("tabid")=='workflowprocess'){
					t.attr("url",'');
					mReload.addClass("disabled");
				}
	...
	_getTabUrlMap: function(tabid){
	    
		var $tab = navTab._getTab(tabid);
	...
	_getTabUrlPara: function(tabid,key){
		var myMap = this._getTabUrlMap(tabid);
	...
	_getAuthId: function(){
		var $tab = this._getTabs().eq(this._currentIndex);
		var myMap = this._getTabUrlMap($tab.attr("tabid"));
		if(myMap)
			return myMap.get("authid");
	},
	_getParam: function(param){
		var $tab = this._getTabs().eq(this._currentIndex);
		var myMap = this._getTabUrlMap($tab.attr("tabid"));
		if(myMap)
			return myMap.get(param);
	},
	...
	_closeTab: function(index, openTabid){
		...
		if($('[name="closeCallBackButton"]',this._getPanels().eq(index))){
			try{$('[name="closeCallBackButton"]',this._getPanels().eq(index)).click();}catch(e){}
		}
	...
	_loadUrlCallback: function($panel){
		...
		$('[authId]', $panel).hide();
		//获取主键
		var para = navTab._getAuthId();
		if(para){
			//ajax提交获取表单数据
			showLoading=false;
			doGetAjax(CPATH.domain_1 + "/rs/toolbar/" + para,{},
					function(data){$('[authId]', $panel).each(function(){
						for(var i = 0;i < data.length;i++){
							if($(this).attr('authId') === data[i]){
								$(this).show();
							}
						}
					});}
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			if(op.fresh || $tab.attr("url") != url) {
				if(op.ismenu && op.ismenu=="true"){
					navTab._switchTab(iOpenIndex);
				}else{
					$tab.attr("url", url);
			...
			if(this._getTabs().length>maxOpenNavTab){
				alertMsg.error('超过窗口打开最大数限制['+maxOpenNavTab+']');
				return;
			}
