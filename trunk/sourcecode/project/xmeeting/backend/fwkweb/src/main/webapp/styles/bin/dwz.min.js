...
if(domain=="1"){
return CPATH.domain_1;}else if(domain=="2"){
return CPATH.domain_2;}else if(domain=="3"){
return CPATH.domain_3;}else if(domain=="4"){
return CPATH.domain_4;}else if(domain=="5"){
return CPATH.domain_5;}else if(domain=="6"){
...
sampleTb.html("");
srow.appendTo(sampleTb);
$.each(data.jsonData.list,function(index,json){
parseRow(srow,json,i);});
srow.pagnationStyle();
srow.hide();}
function istimeout(respJson){
if(DWZ.jsonEval(respJson.responseText).statusCode==DWZ.statusCode.timeout){
if(alertMsg){
alertMsg.error(respJson.message||DWZ.msg("sessionTimout"),{okCall:DWZ.loadLogin});}else{
DWZ.loadLogin();}
return true;}else{
return false;}}
function parseRow(sampleRow,data,i){
var row=sampleRow.clone();
row.children().each(function(){
var $this=$(this);
if($this.attr("type")=='check')
$this.find(':checkbox"').val(data[$this.attr("name")]);
else
$this.text(data[$this.attr("name")]);});
row.css("display","");
row.attr('name','');
row.appendTo(sampleRow.parent());}
var DWZ={
keyCode:{
...
statusCode:{ok:200,error:300,timeout:301,nopermission:401},
ui:{sbar:true},
frag:{},
_msg:{},
_set:{
...
else DWZ.loadLogin();}else if(json.statusCode==DWZ.statusCode.nopermission){
if(json.message)alertMsg.error(json.message);}else{
alertMsg.error("<div>  "+xhr.responseText+"</div>");}}else{
...
if(json.message&&alertMsg)alertMsg.error(json.message);}else if(json.statusCode==DWZ.statusCode.nopermission){
if(json.message)alertMsg.error(json.message);}else if(json.statusCode==DWZ.statusCode.timeout){
if(alertMsg)alertMsg.error(json.message||DWZ.msg("sessionTimout"),{okCall:DWZ.loadLogin});
...
DWZ.loadLogin();}});}
if(json.statusCode==DWZ.statusCode.error){
if(json.message)alertMsg.error(json.message);}else if(json.statusCode==DWZ.statusCode.nopermission){
if(json.message)alertMsg.error(json.message);}else{
...
$this.height($this.attr('customHeight')).css("overflow","auto");}else{
...
return this.replace(RegExp("({[A-Za-z_]+[A-Za-z0-9_]*})","g"),val);},
...
validator.stopRequest(element,valid);},error:function(){
validator.stopRequest(element,true);}},param,previous));
return "pending";},"Please enter a valid date.");
...
var containerOutWidth=$(window).width()-39;
...
if(op.showSub||op.ckbox)tree.subTree(op,op.level+1);}else{
...
try{
resizeMonitorMap();}catch(e){}
try{
resizeDispatchPage();}catch(e){}}
...
if(!$(element).attr("validateRemote")){
...
url=unescape($this.attr("href")).replaceTmByValue($this.attr("transfer"));}else if($this.attr("multiable")==="no"){
...
url=unescape($this.attr("href")).replaceTmByValue($check.val());}else{
...
var $this=$(this);
$this.click(function(){$(this).parent().remove();});});}(function($){
$.fn.extend({
...
menuAction=menu.menuAction;}else{
menuAction=getDomain(menu.domainName)+menu.menuAction;}
htmlStr+='<li><a '+external+' href="'+menuAction+'" target="navTab" rel="'+menu.menuTitle+'">'+menu.menuName+'</a></li>';}
return htmlStr;}
...
if(op.mask){
if(op.zIndex){
$(dialog).css("zIndex",op.zIndex);}else{
...
tbody.wrap("<div class='gridScroller'"+customHeightStr+" "+layoutStr+" style='width:"+$tc.width()+"px; height="+layoutH+"'><div class='gridTbody'><table style='width:"+(tlength-20)+"px;'></table></div></div>");
...
if(json.callBackMethod){
window[json.callBackMethod]();}else if(json.navTabId){
...
if(searchMethod){
window[searchMethod]({pageNum:pageNum});}else{}}});
...
if(searchMethod){
window[searchMethod]({pageNum:event.data.pageNum});}else{}
event.preventDefault();});}},
...
if(searchMethod){
window[searchMethod]({orderField:orderField,orderDirection:orderDirection});}else{}});});},
...
hasNext:function(){
return this.getCurrentPage()<this.numPages();}});})(jQuery);
function printPageBarDiv(pageDiv,totalCountDiv,pageResponse){
totalCountDiv.html('条，�?+pageResponse.totalCount+'�?);
...
killAllBox();}
return false;});
$("#op_"+box.attr("id")).find(">li").comboxOption(selector,box);});},setCurrentVal:function(currentVal){
var $this=$(this);
$this.val(currentVal);
...
$this.changeStyle();});}else if($this.attr("codeType")){
...
$this.changeStyle();});}else{
$this.changeStyle();}});},changeStyle:function(){
var $this=$(this);
...
$refSec.html(html).insertAfter($refSecCombox);
$refSecCombox.remove();
