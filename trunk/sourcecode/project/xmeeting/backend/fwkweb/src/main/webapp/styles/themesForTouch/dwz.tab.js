...
(function($){
	$.fn.extend({
		...
		tabs: function (options){
			var op = $.extend({reverse:false, eventType:"click", currentIndex:0, stTabHeader:"> .tabsHeader", stTab:">.tabsHeaderContent>ul", stTabPanel:"> .tabsContent", ajaxClass:"j-ajax", closeClass:"close", prevClass:"tabsLeft", nextClass:"tabsRight",callback:this.attr("callback")}, options);
			...
			function initTab(jT){
				...
				jTabs.each(function(iTabIndex){
					...
					else $(this).click(function(event){switchTab(jT, iTabIndex,op.callback );});
			...
			function switchTab(jT, iTabIndex,callback ){
				var jSelector = jT.add($("> *", jT));
				var jTabHeader = $(op.stTabHeader, jSelector);
				var jTabs = $(op.stTab + " li", jTabHeader);
				var jGroups = $(op.stTabPanel + " > *", jSelector);
				...
				if (op.reverse && (jTab.hasClass("selected") )) {
					...
					jGroups.hide().eq(op.currentIndex).show(	0,function (){
						
						createScollerByDivObject(jGroups.eq(op.currentIndex));
						 });
					//add by luzhen for showTabContent
					jGroups.eq(op.currentIndex).trigger("showTabContent");
				}
