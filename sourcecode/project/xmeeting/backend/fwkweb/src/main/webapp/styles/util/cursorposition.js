	function getCurPos(_textarea) {
		...
		if ('number' === typeof _textarea.selectionStart) {//chrome 、firefox ……
			...
			if (userRange.parentElement() == _textarea) {
				...
				for (start = 0; textRange.compareEndPoints('StartToStart',
						userRange) < 0
						&& textRange.moveStart('character', 1) !== 0; ++start) {
	...
	function insertStrToTextarea(_textarea, str) {
		...
		var str0 = _textarea.value.substring(0, pos.start), str1 = _textarea.value
				.substring(pos.end);
