...
return obj;}else if(typeof obj.hashCode==FUNCTION){
hashCode=obj.hashCode();
return(typeof hashCode=="string")?hashCode:hashObject(hashCode);}else if(typeof obj.toString==FUNCTION){
return obj.toString();}else{
try{
return String(obj);}catch(ex){
...
return function(kv){
if(kv===null){
throw new Error("null is not a valid "+kvStr);}else if(typeof kv=="undefined"){
...
bucketEntry[1]=value;}else{
bucket.addEntry(key,value);}}else{
...
var c_point=new OpenLayers.LonLat(stationPath[0].lng,stationPath[0].lat).transform(new OpenLayers.Projection("EPSG:4326"),this.map.getProjectionObject());
this.map.setCenter(c_point,this.mapLevel);
var size=new OpenLayers.Size(21,25);
var offset=new OpenLayers.Pixel(-(size.w/2),-size.h);
var icon=new OpenLayers.Icon(this.iconUrl,size,offset);
for(var i=0;i<stationPath.length;i++){
var position=new OpenLayers.LonLat(stationPath[i].lng,
stationPath[i].lat).transform(new OpenLayers.Projection("EPSG:4326"),this.map.getProjectionObject());
...
this.map.addLayer(this.markers);
for(var i=0;i<stationPath.length;i++){
var upStationUrl=this.iconArray[this.stationIconCount]+"/up/marker"+i+".png";
var size=new OpenLayers.Size(21,25);
var offset=new OpenLayers.Pixel(-(size.w/2),-size.h);
var icon=new OpenLayers.Icon(upStationUrl,size,offset);
var marker=new OpenLayers.Marker(stationPath[i],icon);
this.markers.addMarker(marker);}
this.stationIconCount=this.stationIconCount+1;
if(this.upIconCount>4){
this.upIconCount=this.upIconCount-5;}}
Mapbus.prototype.createBusWithoutCallback=function(lng,lat){
this.map.addLayer(this.markers);
var carUrl=this.urlhead_00+this.imgbus_West;
var LonLat=new OpenLayers.Geometry.Point(lng,lat);
this.map.addLayer(this.markers);
var size=new OpenLayers.Size(21,25);
var offset=new OpenLayers.Pixel(-(size.w/2),-size.h);
var icon=new OpenLayers.Icon(carUrl,size,offset);
var marker=new OpenLayers.Marker(LonLat,icon);
this.markers.addMarker(marker);}
Mapbus.prototype.setCenter=function(lonlat){
this.map.setCenter(lonlat);}
Mapbus.prototype.changeCenterByLatlng=function(lonlat){
this.map.moveTo(lonlat);}
Mapbus.prototype.trackplaybackwithoutBreakpoint=function(path,second){
var vmap=this.map;
var index=0;
this.map.addLayer(this.markers);
var size=new OpenLayers.Size(21,25);
var offset=new OpenLayers.Pixel(-(size.w/2),-size.h);
var icon=new OpenLayers.Icon(this.iconUrl,size,offset);
var marker=new OpenLayers.Marker(path[0],icon);
this.markers.addMarker(marker);
this.c=setInterval(function(){
if(index>path.length-1){}
var px=marker.map.getLayerPxFromLonLat(path[index]);
marker.moveTo(px);
vmap.moveTo(path[index]);
index++;},second);}
Mapbus.prototype.clearInterval=function(){
clearInterval(this.c);}
Mapbus.prototype.init=function(isController){
this.layerExtent=new OpenLayers.Bounds(110,30,125,
35);
this.szmapUrl='http://172.29.128.194/szmap';
this.googlemapUrl="http://172.29.128.194/mapserver/tilemaps/${z}/${y}/${x}";
this.proj=new OpenLayers.Projection("EPSG:4326");
this.proj1=new OpenLayers.Projection("EPSG:900913");
this.isController=isController;
this.iconUrl="http://www.openlayers.org/dev/img/marker.png";
this.carUrl_00="/syWeb/styles/openlayer_src/icon/bus/bus_small.png";
this.carUrl_01="/syWeb/styles/openlayer_src/icon/bus/bus_small.png";
this.carUrl_10="/syWeb/styles/openlayer_src/icon/bus/bus_small.png";
this.carUrl_11="/syWeb/styles/openlayer_src/icon/bus/bus_small.png";
this.urlhead_00="/syWeb/styles/openlayer_src/icon/bus/";
this.urlhead_01="/syWeb/styles/openlayer_src/icon/bus/";
this.urlhead_10="/syWeb/styles/openlayer_src/icon/bus/";
this.urlhead_11="/syWeb/styles/openlayer_src/icon/bus/";
this.imgbus_East="bus_small.png";
this.imgbus_EastSouth="bus_small.png";
this.imgbus_north="bus_north.jpg";
this.imgbus_northEast="bus_small.png";
this.imgbus_Northwest="bus_small.png";
this.imgbus_south="bus_small.png";
this.imgbus_southEast="bus_small.png";
this.imgbus_West="bus_small.png";
this.iconArray=[
"/syWeb/styles/openlayer_src/icon/station/black",
"/syWeb/styles/openlayer_src/icon/station/blue",
"/syWeb/styles/openlayer_src/icon/station/green",
"/syWeb/styles/openlayer_src/icon/station/pink",
"/syWeb/styles/openlayer_src/icon/station/red"];
this.upIconCount=0;
this.downIconCount=0;
this.stationIconCount=0;
busTable=new Hashtable();
upStationTable=new Hashtable();
downStationTable=new Hashtable();
upStationPathTable=new Hashtable();
downStationPathTable=new Hashtable();
upLineBusTable=new Hashtable();
downLineBusTable=new Hashtable();
upLinePathTable=new Hashtable();
downLinePathTable=new Hashtable();
selectedTable=new Hashtable();}
Mapbus.prototype.buildMap=function(centerlng,centerLat,containerId){
var options={
maxExtend:this.layerExtent,
...
this.map=new OpenLayers.Map(containerId,options);
var layer=this.createSZMapLayerFounderImpl();
this.map.addLayer(layer);
var c_point=new OpenLayers.LonLat(centerlng,centerLat);
var pj_point=c_point.transform(this.proj,this.map.getProjectionObject());
this.map.setCenter(pj_point,12);
this.registerMapCallback(this.map);
this.landmarkVectorLayer=this.createLandmarkVectorLayer("公交首末�?);
this.map.addLayer(this.landmarkVectorLayer);
this.upLineLayerMaker=this.createBusLineVectorLayer("公交线路");
this.map.addLayer(this.upLineLayerMaker);
this.initStationVectorLayer("站点标识");
this.initBusVectorLayer("公交�?);
this.registerLayerCallback();}
Mapbus.prototype.getBoundOfMap=function(){
var bounds=this.map.getExtent();
bounds.transform(this.map.getProjectionObject(),this.proj);
logger.info("bound to - left: "+bounds.left+" right: "+bounds.right+" top: "+bounds.top+" bottom: "+bounds.bottom);
logger.info("bound to 1-left,bottom : "+bounds.left+","+bounds.bottom);
logger.info("bound to 1-right,top : "+bounds.right+","+bounds.top);
return bounds;}
Mapbus.prototype.getZoomLevelOfMap=function(){
return this.map.getZoom();}
Mapbus.prototype.registerMapCallback=function(map){
map.events.register('zoomend',map,function(evt){
var mapObject=map;
var bounds=mapObject.getExtent();
bounds.transform(mapObject.getProjectionObject(),new OpenLayers.Projection("EPSG:4326"));
logger.warn("zoom to the level: "+mapObject.getZoom());
if(mapObject.getZoom()==0){
mapObject.zoomTo(1);}});}
Mapbus.prototype.createGoogleMapLayer=function(){
var googlelayer=new OpenLayers.Layer.XYZ("Google地图",this.googlemapUrl,{
isBaseLayer:true,
...
units:'m'});
return googlelayer;}
Mapbus.prototype.createSZMapLayer=function(){
var agsTileOrigin=new OpenLayers.LonLat(-400,400);
var mapResolutions=[7.61427507662348E-04,3.80713753831174E-04,1.90356876915587E-04,9.51784384577936E-05,4.75892192288968E-05,2.37946096144484E-05,1.18973048072242E-05,5.9486524036121E-06];
var szmap_layer=new OpenLayers.Layer.ArcGISCache('苏州地图',this.szmapUrl,{
tileOrigin:agsTileOrigin,
resolutions:mapResolutions,
sphericalMercator:true,
maxExtent:this.layerExtent,
...
return szmap_layer;}
Mapbus.prototype.createSZMapLayerFounderImpl=function(){
var agsTileOrigin=new OpenLayers.LonLat(-400,400);
var mapResolutions=[7.61427507662348E-04,3.80713753831174E-04,1.90356876915587E-04,9.51784384577936E-05,4.75892192288968E-05,2.37946096144484E-05,1.18973048072242E-05];
szmap_layer=new OpenLayers.Layer.ArcTileImage('苏州地图',this.szmapUrl,{
tileOrigin:agsTileOrigin,
isBaseLayer:true,
type:"png",
numZoomLevers:7,
resolutions:mapResolutions});
return szmap_layer;}
Mapbus.prototype.drawSingleIconWithText=function(lnglat,grade,text){
var singlePoint=new OpenLayers.Geometry.Point(lnglat.lng,lnglat.lat).transform(this.proj,this.map.getProjectionObject());
var attributes={grade:grade,text:text,styleType:"landmark"};
var feature=new OpenLayers.Feature.Vector(singlePoint,attributes);
return feature;}
Mapbus.prototype.drawSingleLandmark=function(sguid,name,grade,lat,lng){
var lnglat=new Object();
lnglat.lng=lng;
lnglat.lat=lat;
var feature=this.drawSingleIconWithText(lnglat,grade,name);
return feature;}
Mapbus.prototype.drawAllLandmark=function(landmarkArray){
for(var i=0;i<landmarkArray.length;i++){
var landmarkObj=landmarkArray[i];
var sguid=landmarkObj.sguid;
var name=landmarkObj.name;
var grade=landmarkObj.grade;
var lat=landmarkObj.lat;
var lng=landmarkObj.lng;
var callback=landmarkObj.callback;
var feature=this.drawSingleLandmark(sguid,name,grade,lat,lng);
this.landmarkVectorLayer.addFeatures(feature);}(function(){if(window.google&&google.gears){return;}
...
return true;};OpenLayers.Util.createUrlObject=function(url,options){options=options||{};if(!(/^\w+:\/\
...
return new OpenLayers.Pixel((evt.clientX+this.element.scrolls[0])-this.element.offsets[0]-this.element.lefttop[0],(evt.clientY+this.element.scrolls[1])-this.element.offsets[1]-this.element.lefttop[1]);},CLASS_NAME:"OpenLayers.Events"});OpenLayers.Tween=OpenLayers.Class({INTERVAL:10,easing:null,begin:null,finish:null,duration:null,callbacks:null,time:null,interval:null,playing:false,initialize:function(easing){this.easing=(easing)?easing:OpenLayers.Easing.Expo.easeOut;},start:function(begin,finish,duration,options){this.playing=true;this.begin=begin;this.finish=finish;this.duration=duration;this.callbacks=options.callbacks;this.time=0;if(this.interval){window.clearInterval(this.interval);this.interval=null;}
...
return foundLayer;},setLayerZIndex:function(layer,zIdx){layer.setZIndex(this.Z_INDEX_BASE[layer.isBaseLayer?'BaseLayer':'Overlay']+zIdx*5);},resetLayersZIndex:function(){for(var i=0,len=this.layers.length;i<len;i++){var layer=this.layers[i];this.setLayerZIndex(layer,i);}},addLayer:function(layer){for(var i=0,len=this.layers.length;i<len;i++){if(this.layers[i]==layer){var msg=OpenLayers.i18n('layerAlreadyAdded',{'layerName':layer.name});OpenLayers.Console.warn(msg);return false;}}
...
return resolutions;},getResolution:function(){var zoom=this.map.getZoom();return this.getResolutionForZoom(zoom);},getExtent:function(){return this.map.calculateBounds();},getZoomForExtent:function(extent,closest){var viewSize=this.map.getSize();var idealResolution=Math.max(extent.getWidth()/viewSize.w,extent.getHeight()/viewSize.h);return this.getZoomForResolution(idealResolution,closest);},getDataExtent:function(){},getResolutionForZoom:function(zoom){zoom=Math.max(0,Math.min(zoom,this.resolutions.length-1));var resolution;if(this.map.fractionalZoom){var low=Math.floor(zoom);var high=Math.ceil(zoom);resolution=this.resolutions[low]-((zoom-low)*(this.resolutions[low]-this.resolutions[high]));}else{resolution=this.resolutions[Math.round(zoom)];}
...
ring=null;if(polys.length==1){feature.geometry=polys[0];}else{feature.geometry=new OpenLayers.Geometry.MultiPolygon(polys);}}
...
if(oListener[0]==sName&&oListener[1]==fHandler&&oListener[2]==bUseCapture)
break;if(oListener)
this._listeners.splice(nIndex,1);};cXMLHttpRequest.prototype.dispatchEvent=function(oEvent){var oEventPseudo={'type':oEvent.type,'target':this,'currentTarget':this,'eventPhase':2,'bubbles':oEvent.bubbles,'cancelable':oEvent.cancelable,'timeStamp':oEvent.timeStamp,'stopPropagation':function(){},'preventDefault':function(){},'initEvent':function(){}};if(oEventPseudo.type=="readystatechange"&&this.onreadystatechange)(this.onreadystatechange.handleEvent||this.onreadystatechange).apply(this,[oEventPseudo]);for(var nIndex=0,oListener;oListener=this._listeners[nIndex];nIndex++)
if(oListener[0]==oEventPseudo.type&&!oListener[2])(oListener[1].handleEvent||oListener[1]).apply(this,[oEventPseudo]);};cXMLHttpRequest.prototype.toString=function(){return'['+"object"+' '+"XMLHttpRequest"+']';};cXMLHttpRequest.toString=function(){return'['+"XMLHttpRequest"+']';};function fReadyStateChange(oRequest){if(cXMLHttpRequest.onreadystatechange)
...
var matches=href.match(this.regExes.kmlIconPalette);if(matches){var palette=matches[1];var file_extension=matches[2];var x=this.parseProperty(iconNode,"*","x");var y=this.parseProperty(iconNode,"*","y");var posX=x?x/32:0;var posY=y?(7-y/32):7;var pos=posY*8+posX;href="http://maps.google.com/mapfiles/kml/pal"+palette+"/icon"+pos+file_extension;}
...
pos.y+")");}else{var metrics=this.symbolMetrics[src.id];node.firstChild.setAttributeNS(null,"transform","rotate("+rotation+" "+metrics[1]+" "+metrics[2]+")");}}}
if(options.isFilled){node.setAttributeNS(null,"fill",style.fillColor);node.setAttributeNS(null,"fill-opacity",style.fillOpacity);}else{node.setAttributeNS(null,"fill","none");}
if(options.isStroked){node.setAttributeNS(null,"stroke",style.strokeColor);node.setAttributeNS(null,"stroke-opacity",style.strokeOpacity);node.setAttributeNS(null,"stroke-width",style.strokeWidth*widthFactor);node.setAttributeNS(null,"stroke-linecap",style.strokeLinecap||"round");node.setAttributeNS(null,"stroke-linejoin","round");style.strokeDashstyle&&node.setAttributeNS(null,"stroke-dasharray",this.dashStyle(style,widthFactor));}else{node.setAttributeNS(null,"stroke","none");}
if(style.pointerEvents){node.setAttributeNS(null,"pointer-events",style.pointerEvents);}
if(style.cursor!=null){node.setAttributeNS(null,"cursor",style.cursor);}
...
var tilePath='/S'+Math.floor(newParams.scaleindex)+'/'+this.params.basemaplayergroupname+'/R'+tileRowGroup+'/C'+tileColGroup+'/'+(newParams.tilerow%this.params.tileRowsPerFolder)+'_'+(newParams.tilecol%this.params.tileColumnsPerFolder)+'.'+this.params.format;if(this.params.querystring){tilePath+="?"+this.params.querystring;}
...
return this.div;},redraw:function(evt){var lonLat;if(evt==null){this.reset();return;}else{if(this.lastXy==null||Math.abs(evt.xy.x-this.lastXy.x)>this.granularity||Math.abs(evt.xy.y-this.lastXy.y)>this.granularity){this.lastXy=evt.xy;return;}
