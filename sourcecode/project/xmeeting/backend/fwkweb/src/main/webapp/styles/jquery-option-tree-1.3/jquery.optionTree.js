...
$.fn.optionTree = function(tree, options) {
    ...
    var removeNested = function (obj,name) {
        $("select[name^='"+ name + "']",$(obj).parent()).remove();
    };
    ...
    return this.each(function() {
       ...
       if(this.tagName=="INPUT"){
    	   options.hiddenInput=this;
       }
        // remove all dynamic options of lower levels
        removeNested(options.hiddenInput,name);
        ...
        if (typeof tree === "object") { // many options exists for current nesting level
            ...
            .change(function() {
            	setValue(options.hiddenInput, options.empty_value); 
                if (this.options[this.selectedIndex].value !== '') {
                    if ($.isFunction(options.on_each_change)) {
                      removeNested(options.hiddenInput,name + '_');
                    ...
                    if (options.set_value_on === 'each') {
                      setValue(options.hiddenInput, this.options[this.selectedIndex].value);
                    }
                } else {
                  removeNested(options.hiddenInput,name + '_');
                    setValue(options.hiddenInput, options.empty_value);
            ...
            if (options.indexed) {
                ...
                setValue(options.hiddenInput, tree);
            }
        }
    });
