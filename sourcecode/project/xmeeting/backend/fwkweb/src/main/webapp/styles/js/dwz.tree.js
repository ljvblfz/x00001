 ...
 (function($){
 	$.extend($.fn, {
		jTree:function(options) {
			var op = $.extend({checkFn:null, selected:"selected", exp:"last_expandable", coll:"last_collapsable", firstExp:"first_expandable",
				firstColl:"first_collapsable", lastExp:"last_expandable", lastColl:"last_collapsable", 
				folderExp:"folder_expandable", folderColl:"folder_collapsable", endExp:"end_expandable", 
				endColl:"end_collapsable",file:"file",ck:"checked", unck:"unchecked"}, options);
			return this.each(function(){
				...
				$(">li", $this).each(function(){
					...
					$li.genTree({
						...
						disabled:$this.hasClass("disabled"),	//增加是否是只读状态
						options: op,
						level: 0,
		...
		genTree:function(options) {
			...
			return this.each(function(){
				...
				var disabled=(op.disabled?"disabled":"");
				if(op.ckbox) {
					if($(">.checked",parent).size() > 0) checked = 'checked';
				}
				if (tree.size()>0) {
					node.children(":first").wrap("<div></div>");
					$(">div", node).prepend("<div class='" + (op.showSub ? op.coll : op.exp) + "'></div>"+(op.ckbox ?"<div class='ckbox " + checked +" "+disabled+  "'></div>":"")+(op.icon?"<div class='"+ (op.showSub ? op.options.folderColl : op.options.folderExp) +"'></div>":""));
					...
					if(op.showSub||op.ckbox) tree.subTree(op, op.level + 1);	//checkbox树直接全部初始化
				} else {
					node.children().wrap("<div></div>");			
					$(">div", node).prepend("<div class='node'></div>"+(op.ckbox?"<div class='ckbox "+checked+" "+disabled+"'></div>":"")+(op.icon?"<div class='file'></div>":""));
					addSpace(op.level, node);
		...
		_check:function(op) {
			...
			if(!op.disabled){					//disabled 时不添加点击改变checked状态的事件
