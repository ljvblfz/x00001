	...
	$.fn.datepicker = function(opts){
		...
		return this.each(function(){
			...
			if($this.attr("minDate")){
				minDate=$this.attr("minDate").parseDate("yyyy-MM-dd");
			}
			function generateCalendar(dp){
				...
				if (startDay > 0){
					...
					for(var t=prevDateWrap.days-startDay+1;t<=prevDateWrap.days;t++) {
						var selectedDate=new Date(dw.year, dw.month-2, t+1);
						if(minDate&&minDate>selectedDate)
							dayStr+='<dd class="disabled" chMonth="-1" day="' + t + '">'+t+'</dd>';
						else
							dayStr+='<dd class="other" chMonth="-1" day="' + t + '">'+t+'</dd>';
					}
				}
				for(var t=1;t<=dw.days;t++){
					var selectedDate=new Date(dw.year, dw.month-1, t+1);
					if(t==dw.day){
						if(minDate&&minDate>selectedDate)
							dayStr+='<dd class="disabled" day="' + t + '">'+t+'</dd>';
						else
				...
				for(var t=1;t<=42-startDay-dw.days;t++){
					var selectedDate=new Date(dw.year, dw.month, t+1);
					if(minDate&&minDate>selectedDate)
						dayStr+='<dd class="disabled" chMonth="1" day="' + t + '">'+t+'</dd>';
					else
						dayStr+='<dd class="other" chMonth="1" day="' + t + '">'+t+'</dd>';
