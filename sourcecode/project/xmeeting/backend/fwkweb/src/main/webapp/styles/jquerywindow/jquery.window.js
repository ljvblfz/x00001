...
/** 
founder change history

1)luzhen 2012-03-31 
add the dock into the instance attribute,but not support the dockaArea in the instance attribute.
The impacting function is getCssStyleByDock and _adjustMinimizedPos.
dock:left or dock:right 
...
$.Window = (function()  {
	...
	function getCssStyleByDock(parent, miniIndex,dock) {
		var targetCss = {};
		...
		}else if( setting.dock == 'left' || setting.dock == 'right' ) {
			targetCss.width = narrow;
			targetCss.height = long - 1;
			targetCss.top = miniIndex * long;
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			dock: "right",				  //added by luzhen for instance
		...
		function _adjustMinimizedPos(bImmediate, callback) { 
			animating = true;
			targetCssStyle = getCssStyleByDock(caller, miniStackIndex,options.dock);
			if( bImmediate ) {
				container.css(targetCssStyle);
				animating = false;
				if( $.isFunction(callback) ) {
