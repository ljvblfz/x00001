...
AmCharts.findPosX = function(a) {
    var b = a.offsetLeft;
    if (a.offsetParent) for (; a = a.offsetParent;) b += a.offsetLeft,
    a != document.body && a != document.documentElement && (b -= a.scrollLeft);
    return b
};
AmCharts.findPosY = function(a) {
    var b = a.offsetTop;
    if (a.offsetParent) for (; a = a.offsetParent;) b += a.offsetTop,
...
AmCharts.AmLegend = AmCharts.Class({
    ...
    createMarker: function(a, b, d) {
        ...
        case "triangleDown":
            ...
            break;
        case "bubble":
            g = AmCharts.circle(f, e / 2, b, d, i, h, j, !0),
            g.translate(e / 2, e / 2)
        }
        return g
    },
...
AmCharts.AmCoordinateChart = AmCharts.Class({
    ...
    parseSerialData: function() {
        ...
        for (b = 0; b < a.length; b++) m = a[b],
        m.dataProvider && this.parseGraphData(m)
    },
    ...
    draw: function() {
        ...
        if (this.logarithmic && (0 >= this.getMin(0, this.data.length - 1) || 0 >= this.minimum)) this.fire("logarithmicAxisFailed", {
            ...
            if (!isNaN(this.min) && !isNaN(this.max) && this.foundGraphs && Infinity != this.min && -Infinity != this.max) {
                ...
                t = !1;
                for (e = p; e < q; e += r) v = AmCharts.roundTo(this.step * e + this.min, d),
                -1 != String(v).indexOf("e") && (t = !0, String(v).split("e"));
                this.duration && (this.maxInterval = AmCharts.getMaxInterval(this.max, this.duration));
                for (e = p; e < q; e += r) if (p = this.step * e + this.min, p = AmCharts.roundTo(p, this.maxDecCount + 1), !(this.integersOnly && Math.round(p) != p)) { ! 0 == i && (0 == p && (p = this.minReal), 2 < n && (p = Math.pow(10, e)), t = -1 != String(p).indexOf("e") ? !0 : !1);
            ...
            a = this.labelsSet;
            this.positionTitle();
            "radar" != j ? (j = this.viX, e = this.viY, b.translate(j, e), a.translate(j, e)) : i.toFront(); ! this.visible || h ? (b.hide(), i.hide(), a.hide()) : (b.show(), i.show(), a.show())
        }
    },
...
AmCharts.CategoryAxis = AmCharts.Class({
    ...
    draw: function() {
        ...
        h = t = d = NaN,
        j.toCategory && (h = b.getCategoryIndexByValue(j.toCategory), isNaN(h) || (d = this.getCoordinate(h - f), k = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, j), this.pushAxisItem(k))),
        j.category && (h = b.getCategoryIndexByValue(j.category), isNaN(h) || (t = this.getCoordinate(h - f), h = (d - t) / 2, k = new this.axisItemRenderer(this, t, j.label, !0, NaN, h, j), this.pushAxisItem(k))),
        j.toDate && (this.equalSpacing ? (h = b.getClosestIndex(this.data, "time", j.toDate.getTime(), !1, 0, this.data.length - 1), isNaN(h) || (d = this.getCoordinate(h - f))) : d = (j.toDate.getTime() - this.startTime) * this.stepWidth, k = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, j), this.pushAxisItem(k)),
        j.date && (this.equalSpacing ? (h = b.getClosestIndex(this.data, "time", j.date.getTime(), !1, 0, this.data.length - 1), isNaN(h) || (t = this.getCoordinate(h - f))) : t = (j.date.getTime() - this.startTime) * this.stepWidth, h = (d - t) / 2, k = "H" == this.orientation ? new this.axisItemRenderer(this, t, j.label, !1, 2 * h, NaN, j) : new this.axisItemRenderer(this, t, j.label, !1, NaN, h, j), this.pushAxisItem(k)),
        d = new this.guideFillRenderer(this, t, d, j),
        t = d.graphics(),
        this.pushAxisItem(d),
        j.graphics = t,
        t.index = g,
        j.balloonText && this.addEventListeners(t, j);
        this.axisCreated = !0;
        b = this.x;
        f = this.y;
...
AmCharts.RecItem = AmCharts.Class({
    construct: function(a, b, d, e, f, g, h, i, j) {
        ...
        if ("H" == V) {
            ...
            N = -3;
            "right" == O && (N += G);
            if (W && (C > ea + 1 || C < N)) W.remove(),
            W = null
        }
        ka && ka.translate(z, Z); ! 1 == a.visible && (ka && ka.remove(), W && (W.remove(), W = null));
        W && (W.attr({
            "text-anchor": U
...
AmCharts.RadItem = AmCharts.Class({
    construct: function(a, b, d, e, f, g, h) {
        ...
        b = a.y - b;
        if ("polygons" == v) {
            for (var P = [], G = [], V = a.data.length, d = 0; d < V; d++) i = 180 - 360 / V * d,
            P.push(b * Math.sin(i / 180 * Math.PI)),
            G.push(b * Math.cos(i / 180 * Math.PI));
            P.push(P[0]);
            G.push(G[0]);
        ...
        if (1 == f && 0 < w && !h) {
            h = a.previousCoord;
            if ("polygons" == v) {
                for (d = V; 0 <= d; d--) i = 180 - 360 / V * d,
                P.push(h * Math.sin(i / 180 * Math.PI)),
                G.push(h * Math.cos(i / 180 * Math.PI));
...
AmCharts.AmGraph = AmCharts.Class({
    ...
    createXYGraph: function() {
        ...
        for (var f = this.start; f <= this.end; f++) {
            ...
            h = d.getCoordinate(i),
            k = e.getCoordinate(j);
            if (!isNaN(i) && !isNaN(j) && (a.push(h), b.push(k), (i = this.createBullet(g, h, k, f)) || (i = 0), j = this.labelText)) g = this.createLabel(g, h, k, j),
            this.positionLabel(h, k, g, this.labelPosition, i)
        }
    ...
    createSerialGraph: function() {
                    ...
                    if (!isNaN(S)) {
                        ...
                        if (ja) {
                            ...
                            if (!Y) {
                                ...
                                if ("regular" == v || "100%" == v) D = "middle",
                                w = l ? 0 < qa.value ? (this.end + 1 - L) * u + b: (this.end + 1 - L) * u - b: 0 < qa.value ? u * L + b: u * L - b;
        ...
        if ("line" == g || "step" == g || "smoothedLine" == g)"smoothedLine" == g ? this.drawSmoothedGraph(q, n, ga, ha) : this.drawLineGraph(q, n, ga, ha),
        Y || this.launchAnimation()
    },
    ...
    drawLineGraph: function(a, b, d, e) {
        if (1 < a.length) {
            ...
            if (0 < f && (j = a.join(";").split(";"), k = b.join(";").split(";"), "serial" == this.chartType && (0 < d.length ? (d.reverse(), e.reverse(), j = a.concat(d), k = b.concat(e)) : this.rotate ? (k.push(k[k.length - 1]), j.push(n), k.push(k[0]), j.push(n), k.push(k[0]), j.push(j[0])) : (j.push(j[j.length - 1]), k.push(n), j.push(j[0]), k.push(n), j.push(a[0]), k.push(k[0]))), a = AmCharts.polygon(g, j, k, m, f, 0, 0, 0, this.gradientRotation), h.push(a), p || void 0 != r)) q || (q = f),
            p || (p = r),
            g = AmCharts.polygon(g, j, k, p, q, 0, 0, 0, this.gradientRotation),
            i.push(g);
            this.applyMask(i, h)
        }
    },
    ...
    drawSmoothedGraph: function(a, b, d, e) {
        if (1 < a.length) {
            ...
            if (0 < l && (k = a.join(";").split(";"), s = b.join(";").split(";"), j = "", 0 < d.length ? (d.reverse(), e.reverse(), k = a.concat(d), s = b.concat(e)) : (this.rotate ? (j += " L" + n + "," + b[b.length - 1], j += " L" + n + "," + b[0]) : (j += " L" + a[a.length - 1] + "," + n, j += " L" + a[0] + "," + n), j += " L" + a[0] + "," + b[0]), d = new AmCharts.Bezier(g, k, s, NaN, 0, 0, m, l, f, j), h.push(d.path), p || void 0 != r)) q || (q = l),
            p || (p = r),
            a = new AmCharts.Bezier(g, a, b, NaN, 0, 0, p, q, f, j),
            i.push(a.path);
            this.applyMask(i, h)
        }
    },
    ...
    createBullet: function(a, b, d) {
        ...
        h = f = 0;
        if (this.customBullet || a.customBullet) k = this.customBullet,
        a.customBullet && (k = a.customBullet),
        ...
        if (i) {
            ...
            if ("serial" == this.chartType && (0 > b - f || b - f > this.width || d < -g / 2 || d - h > this.height)) i.remove(),
            i = null;
        ...
        if (this.enabled) {
            ...
            if (d.mouseIsOver || v || D || this.forceShow) if (this.forceShow = !1, D) {
                ...
                if (K && this.valueBalloonsEnabled && u && d.balloon.enabled) {
                    ...
                    Y, u = 0; u < K.length; u++) s = K[u],
                    s.showBalloon && (!s.hidden && s.balloonText) && (t = r.axes[s.valueAxis.id].graphs[s.id], w = t.y, isNaN(w) || (q ? Math.abs(e - w) < i && (i = Math.abs(e - w), Y = s) : Math.abs(f - w) < i && (i = Math.abs(f - w), Y = s)));
        ...
        if (a) {
            ...
            if (this.enabled && 0 < this.data.length) {
                if (this.pan) this.rolledOver = !1;
                    ...
                    e.chart = this.chart;
                    if ("cursor" == this.type) this.rotate ? this.selectionPosY = a = d: this.selectionPosX = a = b,
